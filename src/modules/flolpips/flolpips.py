...
import modules.flolpips.pretrained_networks as pn
import modules.flolpips.utils as utils
# from modules.flolpips.pwcnet import Network as PWCNet
...
class Flolpips(nn.Module):
    ...
    def forward(self, I0, I1, frame_dis, frame_ref):
        ...
        flow_ref = self.flownet.get_optical_flow(frame_ref, I0)
        flow_dis = self.flownet.get_optical_flow(frame_dis, I0)
        flow_diff = flow_ref - flow_dis
        flolpips_wrt_I0 = self.loss_fn.forward(frame_ref, frame_dis, flow_diff, normalize=True)

        flow_ref = self.flownet.get_optical_flow(frame_ref, I1)
        flow_dis = self.flownet.get_optical_flow(frame_dis, I1)
        flow_diff = flow_ref - flow_dis
