...
from .cuda_inference import build_index_dec, build_index_enc
...
class GaussianEncoder(AEHelper):
    ...
    def build_indexes_decoder(self, scales, skip_thres=None):
        scales = scales.reshape(-1)
        indexes, skip_cond = build_index_dec(scales, self.scale_min, self.scale_max,
                                             self.log_scale_min, self.log_step_recip, skip_thres)
        if skip_thres is not None:
            indexes = indexes[skip_cond]
        return indexes, skip_cond

    def build_indexes_encoder(self, symbols, scales, skip_thres=None):
    ...
    def encode_y(self, x, scales, skip_thres=None):
        symbols = self.build_indexes_encoder(x, scales, skip_thres)
    ...
    def decode_and_get_y(self, scales, dtype, device, skip_thres=None):
        indexes, skip_cond = self.build_indexes_decoder(scales, skip_thres)
