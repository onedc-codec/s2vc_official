...
#
# Slimmed for OneDC-video inference release: kept only the building blocks
# (ResidualBlock*/DepthConv*/ConvFFN*/DepthConvBlock4/5) imported by the
# inference path; LPIPS / VGG / Transformer helpers removed.
...
class DepthConv3(nn.Module):
    """Variable kernel_size and channel expansion."""
    def __init__(self, in_ch, out_ch, slope=0.01, inplace=False, kernel_size=3, ch_expansion=1.5):
        super().__init__()
        mid_ch = int(in_ch * ch_expansion)
        self.convs = nn.Sequential(
            nn.Conv2d(in_ch, mid_ch, 1),
            nn.LeakyReLU(negative_slope=slope, inplace=inplace),
            nn.Conv2d(mid_ch, mid_ch, kernel_size, padding=kernel_size//2, groups=mid_ch),
...
class DepthConvBlock5(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, slope_depth_conv=0.01, inplace=False, mlp_ratio=2.0):
        super().__init__()
        self.block = nn.Sequential(
            DepthConv3(in_ch, in_ch, slope=slope_depth_conv, inplace=inplace, kernel_size=5, ch_expansion=mlp_ratio),
            ConvFFN4(in_ch, inplace=inplace, expansion_factor=mlp_ratio),
            DepthConv(in_ch, out_ch, slope=slope_depth_conv, inplace=inplace),
